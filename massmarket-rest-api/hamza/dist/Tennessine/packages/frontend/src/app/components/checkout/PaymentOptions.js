...
const PaymentOptions = ({ next, imgSrc, totalDollar, purchaseAddress, cryptoTotal, }) => {
    const [showPaymentOptions, setShowPaymentOptions] = useState(false);
    const [showModal, setShowModal] = useState(null);
    return (<div>
      <FullModal isOpen={Boolean(showModal)}>
        <QRScan imgSrc={imgSrc} totalDollar={totalDollar} purchaseAddress={purchaseAddress} cryptoTotal={cryptoTotal} showModal={showModal} goBack={() => setShowModal(null)}/>
      </FullModal>
      ...
      <WalletConnectQR purchaseAddress={purchaseAddress} displayedTotal={cryptoTotal}/>
      ...
      <div className={`${showPaymentOptions ? "flex gap-4 mt-8" : "hidden"}`}>
        <SecondaryButton onClick={() => setShowModal("QR")}>
          <div className="flex gap-2">
            ...
            <Image src={"/assets/qr-code.svg"} width={15} height={15} alt="item-thumbnail" unoptimized={true}/>
          </div>
        </SecondaryButton>
        <SecondaryButton onClick={() => setShowModal("Address")}>
          <div className="flex gap-2">
            <h6> Copy Address</h6>
            <Image src={"/assets/clipboard.svg"} width={15} height={15} alt="item-thumbnail" unoptimized={true}/>
          </div>
        </SecondaryButton>
