...
const CheckoutFlow = () => {
    ...
    const [checkoutReqId, setCheckoutRequestId] = useState(null);
    const [showErrorMessage, setShowErrorMessage] = useState(null);
    const [erc20Checkout, setErc20Checkout] = useState(false);
    const [cryptoTotal, setCryptoTotal] = useState(null);
    const [purchaseAdd, setPurchaseAdd] = useState(null);
    const [totalDollar, setTotalDollar] = useState(null);
    ...
    useEffect(() => {
        if (finalizedOrders && checkoutReqId) {
            ...
            if (erc20Checkout) {
                setSrc(`ethereum:${erc20Addr}/transfer?address=${purchaseAddress}&uint256=${totalInCrypto}`);
            }
            else {
    ...
    const renderContent = () => {
        if (step === 0) {
            ...
            purchaseAdd &&
            cryptoTotal) {
            return (<PaymentOptions next={() => {
                    setStep(3);
                }} imgSrc={imgSrc} totalDollar={totalDollar} purchaseAddress={purchaseAdd} cryptoTotal={cryptoTotal}/>);
        }
        else {
          ...
          <div className="flex-col items-center gap-2">
            ...
            <div className="bg-white w-fit p-2 border-2 rounded-xl shadow-lg flex gap-2">
              <p>0xb5c8 ... 9838</p>
              <Image src={"/assets/copy-icon.svg"} width={15} height={15} alt="item-thumbnail" unoptimized={true}/>
            </div>
          </div>
        </div>);
        }
    ...
    return (<main className="pt-under-nav h-screen bg-gray-100 ">
      ...
      <div className="px-5">
        <div>
          <ProgressBar allSteps={["Review Cart", "Shipping Details", "Payment", "Success"]} currentStep={step}/>
          {renderContent()}
        </div>
      </div>
    </main>);
