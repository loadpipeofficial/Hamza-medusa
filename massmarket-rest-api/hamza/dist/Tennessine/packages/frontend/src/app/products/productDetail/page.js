...
const ProductDetail = () => {
    const { products, updateOrder, orderItems, orderId, addProductToTag, allTags, } = useStoreContext();
    ...
    const [buttonState, setButton] = useState("Review");
    ...
    const handleDelete = async () => {
        ...
        if (!res || res.error) {
            setShowErrorMessage(res?.error || "There was an error removing tag from Item.");
        }
        else {
    ...
    // }, [flyoutRef]);
    useEffect(() => {
        const itemsInCurrentCart = (orderId && orderItems.get(orderId)?.items) || null;
    ...
    useEffect(() => {
        if (itemId) {
            ...
            const qty = _item && orderId ? orderItems.get(orderId)?.items?.[itemId] || 0 : 0;
            setQuantity(qty);
        }
    }, [itemId]);
    const confirmDelete = (<div id="confirm-modal-container" className="absolute h-4/6 w-full flex justify-center items-center">
      <div id="confirm-modal" className="w-72 h-64 bg-white text-center px-6 py-2 flex items-center justify-center rounded-2xl flex-col border">
        <p>Are you sure you want to remove this product from your store?</p>
        <div id="buttons" className="flex text-sm gap-2 mt-4">
          <button className="border-4 p-2 rounded-lg" onClick={() => setShowConfirmModal(false)}>
            Cancel
          </button>
          <button onClick={handleDelete} className="bg-blue-900 p-3 text-white rounded-lg">
            Im sure
          </button>
        </div>
      </div>
    ...
    const getCtaButton = () => {
        if (!addedToCart) {
            return (<Button disabled={!quantity} onClick={async () => {
          ...
          {(Number(item.price) * quantity).toFixed(2)}
            ...
            return (<button className="flex justify-center bg-green-600 w-full text-white px-4 rounded-md">
          <Image src="/assets/checkmark.svg" alt="checkmark-icon" width={24} height={24}/>
        </button>);
        }
        else
    ...
    return (<main className="pt-under-nav h-screen bg-gray-100">
      {showConfirmModal && confirmDelete}
      <ModalHeader />
      <section className="h-[45rem] flex flex-col">
        <ErrorMessage errorMessage={showErrorMessage} onClose={() => {
        ...
        src={item.metadata.image} alt="product-detail-image" width={136} height={136} className="border rounded-lg" unoptimized={true} style={{ maxHeight: "136px", maxWidth: "136px" }}/>)}
              ...
              <div>
                ...
                <Dropdown>
                  ...
                  <DropdownMenu className="bg-white">
                    {dropdownOpts.map((a) => {
            return (<DropdownItem key={a} onClick={() => setQuantity(a)}>
                          {a}
                        </DropdownItem>);
        })}
                  </DropdownMenu>
