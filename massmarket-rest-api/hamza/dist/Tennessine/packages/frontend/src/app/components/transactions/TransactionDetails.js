...
const TransactionDetails = ({ closeDetails, order, opened, transactionHash, }) => {
    if (!opened)
        return null;
    ...
    const renderProduct = (itemId, quantity) => {
        const product = products.get(itemId);
        return (<div key={product.metadata.title} className="flex gap-1">
        {product.metadata.image && (<div className="border p-1 rounded">
            <Image src={product.metadata.image} width={64} height={64} alt="item-thumbnail" unoptimized={true}/>
          </div>)}
    ...
    return (<FullModal isOpen={opened} onClose={closeDetails} header="Transaction Details" modalBgColor="gray-100">
      <section>
        ...
        <section className="mt-4 flex flex-col gap-8 bg-white">
          <div className="mx-4 py-4 flex flex-col gap-3">
            {items.map((i) => renderProduct(i[0], i[1]))}
          </div>
        </section>
        <section id="payment" className="mt-8">
          ...
          <div className="p-4 bg-white flex gap-2 items-center">
            <Image src={`/assets/dollar-avatar.svg`} width={32} height={32} alt="item-thumbnail" unoptimized={true}/>
            <p>{symbol}</p>
