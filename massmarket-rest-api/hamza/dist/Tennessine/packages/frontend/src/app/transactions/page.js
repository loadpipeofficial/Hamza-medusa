...
function Transactions() {
    ...
    const handleTransactionsClick = (order, hash) => {
        setSelectedOrder(order);
        hash && setSelectedTxHash(hash);
    };
    return (<main className="bg-gray-100 pt-under-nav">
      {selectedOrder && (<TransactionDetails opened={!!selectedOrder} order={selectedOrder} closeDetails={() => setSelectedOrder(null)} transactionHash={selectedTxHash || null}/>)}
        ...
        <section>
          ...
          <section className="mt-6 flex flex-col-reverse gap-2">
            ...
            ? transactions.map((entry) => {
                ...
                if (!allItemsObj?.items)
                    return null;
                return (<div key={cartId}>
                      <TransactionBlock order={allItemsObj.items} onClick={handleTransactionsClick} orderStatus={allItemsObj.status || IStatus.Pending} transactionHash={transactionHash} searchPhrase={searchPhrase}/>
                    </div>);
            })
            : null}
          </section>
