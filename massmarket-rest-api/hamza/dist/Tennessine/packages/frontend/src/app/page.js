...
function Homepage() {
    ...
    const connectAction = isAuthenticated === IStatus.Complete
        ? () => router.push("/products")
        : inviteSecret
    ...
    const welcomeCopy = inviteSecret
        ...
        : "Welcome back";
    if (openSignupModal) {
        return (<SignUp backToLanding={() => {
                setOpenSignupModal(false);
            }}/>);
    }
    else if (openConnectModal) {
        return (<ConnectWallet close={() => {
                setOpenConnectModal(false);
            }}/>);
    }
    return (<main className="min-h-screen overflow-hidden flex flex-col">
      <Account />
      ...
      <section className="bg-gray-300 p-8 mt-auto">
        ...
        <div>
          <Button onClick={connectAction} color={"bg-gradient-to-r from-button-gradient-start to-button-gradient-end"}>
            Connect Wallet
          </Button>
        </div>
      </section>
