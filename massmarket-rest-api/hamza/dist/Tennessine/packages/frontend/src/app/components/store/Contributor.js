...
const Contributor = ({ contributor, openHalfModal, }) => {
    return (<div key={contributor.name} className="py-4 border-b only:border-0 flex gap-4">
      <div id="container avatar" className="flex justify-center relative">
        ...
        <div id="overlay" className="absolute margin-auto top-0 bottom-0 left-0 right-0 flex justify-center items-center text-gray-500">
      ...
      <div className="ml-auto">
        <button onClick={openHalfModal}>
          <Image src="/assets/remove.svg" width={24} height={24} alt="remove-icon"/>
        </button>
      </div>
    </div>);
};
