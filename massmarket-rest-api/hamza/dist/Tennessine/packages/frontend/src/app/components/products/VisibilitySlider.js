...
import { SELECT_TAG, DESELECT_TAG, } from "@/reducers/tagReducers";
const VisibilitySlider = ({ selectedTagsDispatch, itemId, selectedTags, }) => {
    const { publishedTagId, allTags, addProductToTag, removeProductFromTag } = useStoreContext();
    ...
    useEffect(() => {
        const isVisible = publishedTagId && _selectedTags
    ...
    return (<section>
      ...
      <div className="slider flex justify-between border rounded-lg bg-white text-center">
        <div className={`px-2 py-4 grow ${selectedOption === 1 ? "bg-gray-200" : ""}`}>
          <button onClick={handleVisibility}>Visible</button>
        </div>
        <div className={`px-2 py-4 grow ${selectedOption === 2 ? "bg-gray-200" : ""}`}>
          <button onClick={handleHideTag}>Hidden</button>
        </div>
        <div className={`px-2 py-4 grow ${selectedOption === 3 ? "bg-gray-200" : ""}`}>
          <button onClick={() => {
...
//# sourceMappingURL=VisibilitySlider.js.map