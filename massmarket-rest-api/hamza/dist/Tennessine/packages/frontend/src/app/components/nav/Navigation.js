...
const Navigation = () => {
    ...
    const renderItems = () => {
        return _menuOptions.map((opt, i) => {
            return (<section data-testid={`menu-button-${opt.title}`} key={i} onClick={() => setMenuOpened(false)}>
          <div className="flex">
    ...
    return menuOpened ? (<FullModal isOpen={menuOpened}>
      <main>
        <div className="w-full border border-gray-200 p-4 text-base flex justify-between">
          ...
          <div className="flex gap-4">
            <button onClick={menuSwitch}>
              <Image src={`/assets/MassLabsLogo.svg`} width={24} height={24} alt="profile-avatar"/>
            </button>
          </div>
        </div>
            ...
            <div className="flex text-xs gap-1">
              <SecondaryButton>
                <div className="flex items-center gap-1">
                  Go to Shop
                  <Image src="/assets/forward-button.svg" width={12} height={12} alt="forward-icon"/>
                </div>
              </SecondaryButton>
              <SecondaryButton>
                <Link href={`/products/edit?${createQueryString("itemId", "new", searchParams)}`} onClick={menuSwitch}>
                  Add Product +
                </Link>
              </SecondaryButton>
              <SecondaryButton>
                <div className="flex items-center gap-1">
                  Settings
                  <Image src="/assets/settings.svg" width={12} height={12} alt="settings-icon"/>
                </div>
              </SecondaryButton>
            </div>
          </div>
    ...
    </FullModal>) : (<div className={`absolute left-0 top-0 right-0`}>
      <div className="w-full border border-gray-200 p-4 text-base flex justify-between">
        <div className="flex items-center text-primary-gray" onClick={menuSwitch}>
          {loggedIn ? (<div className="flex gap-2">
              <Image src="/assets/back-button.svg" width={12} height={12} alt="hamburger-icon" className="h-6"/>
              <p>back</p>
            </div>) : null}
