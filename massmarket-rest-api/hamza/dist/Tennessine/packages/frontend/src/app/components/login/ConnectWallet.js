...
const ConnectWallet = ({ close }) => {
    ...
    const { walletAddress, inviteSecret, relayClient, setWallet, setKeyCardEnrolled, } = useMyContext();
    const { isAuthenticated, setIsAuthenticated } = useAuth();
    const enrollKeycard = useRef(false);
    const redeemSecret = useRef(false);
    const { data: _wallet, status: walletStatus } = useWalletClient();
    const [connectionStatus, setConnectionStatus] = useState(IStatus.Pending);
    ...
    useEffect(() => {
        if (newClerk &&
            ...
            (async () => {
                ...
                const hash = await relayClient.redeemInviteSecret(inviteSecret, clientWallet);
    ...
    useEffect(() => {
        if ((returningClerk || (newClerk && hasAccess)) &&
            relayClient &&
    ...
    const displayConnectors = () => {
        return connectors.map((connector) => (<button key={connector.uid} onClick={() => connect({ connector })} className="p-4 bg-white my-4 border rounded w-full">
        {connector.name}
      </button>));
    };
    ...
    return (<section className="bg-gray-100 h-screen absolute top-0	right-0	left-0">
      <div className="h-fit w-full border border-gray-200 p-4 text-base flex justify-between">
        <div className="flex">
          <Image src="/assets/wallet.svg" width={24} height={24} alt="hamburger-icon" className="h-6"/>
