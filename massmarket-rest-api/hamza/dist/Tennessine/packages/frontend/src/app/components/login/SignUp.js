...
const SignUp = ({ backToLanding }) => {
    ...
    if (openConnectModal)
        return <ConnectWallet close={backToLanding}/>;
    return (<section className="bg-gray-100 h-screen absolute top-0	right-0	left-0">
      <div className="h-fit w-full p-4 text-base flex justify-between">
        <Image src="/assets/arrow-left.svg" width={24} height={24} alt="arrow-icon" className="h-6" onClick={backToLanding}/>
      </div>
      <section className="mx-4 my-6">
        <form className="flex flex-col" onSubmit={(e) => {
          ...
          <input className="border-2 border-solid mt-1 p-2" id="fname" name="fname" onChange={(e) => {
            e.preventDefault();
        }}/>
        </form>
