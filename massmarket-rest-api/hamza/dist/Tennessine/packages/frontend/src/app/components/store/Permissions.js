...
const Permissions = ({ close }) => {
    ...
    const copyToClipboard = () => {
        inviteLink &&
            navigator.clipboard.writeText(`${process.env.NODE_ENV === "development" ? "localhost:3000" : "mass.market"}?inviteSecret=${inviteLink}`);
    };
    const renderList = (arr) => {
        return arr.map((a) => {
            return (<ContributorBlock contributor={a} key={a.name} openHalfModal={openHalfModal}/>);
        });
    };
    const confirmModal = (<HalfModal isOpen={isHalfModalOpen} onClose={closeHalfModal}>
    ...
    return (<section className="pt-under-nav">
      {confirmModal}
      ...
      <section className="m-4">
        <section id="add-contributors" className="border-b pb-10">
          ...
          <div className="mt-6">
            {showInviteLink && inviteLink ? (<div className="flex">
