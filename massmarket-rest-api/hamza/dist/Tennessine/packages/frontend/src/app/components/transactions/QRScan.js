...
function QRScan({ imgSrc, totalDollar, purchaseAddress, showModal, cryptoTotal, goBack, }) {
    console.log("for testing send payment to:", imgSrc);
    const { orderId, orderItems, setOrderId } = useStoreContext();
    const [src, setQr] = useState(null);
    ...
    return (<div id="QR-modal" className="text-center h-full flex items-center justify-center">
      {showModal === "QR" ? (<div className="flex flex-col items-center gap-8">
          ...
          <h2>{amount}</h2>
        </div>) : (<div className="flex flex-col gap-5 text-primary-gray">
