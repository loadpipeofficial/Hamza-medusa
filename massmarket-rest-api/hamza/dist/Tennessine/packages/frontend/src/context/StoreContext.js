...
import { productReducer, ADD_PRODUCT, UPDATE_PRODUCT, ADD_PRODUCT_TAGS, REMOVE_PRODUCT_TAG, SET_PRODUCTS, UPDATE_PRICE, UPDATE_METADATA, UPDATE_STOCKQTY, } from "@/reducers/productReducers";
import { allTagsReducer, ADD_TAG, SET_ALL_TAGS } from "@/reducers/tagReducers";
import { buildState } from "@/utils/buildState";
import { CLEAR_ORDER, REMOVE_ORDER_ITEM, UPDATE_ORDER_ITEM, UPDATE_ORDER_STATUS, SET_ALL_ORDER_ITEMS, orderReducer, } from "@/reducers/orderReducers";
...
export const StoreContext = createContext(initialStoreContext);
export const StoreContextProvider = (props) => {
    ...
    const [publishedTagId, setPublishedTagId] = useState(null);
    const [finalizedOrders, setFinalizedOrders] = useReducer(finalizedOrderReducer, new Map());
    ...
    const [relays, setRelays] = useState(dummyRelays);
    ...
    useEffect(() => {
        ...
        (async () => {
            if (db) {
                const productsLocal = (await getParsedMapData("products", db));
                const tagsLocal = (await getParsedMapData("tags", db));
                const orderItemsLocal = (await getParsedMapData("orderItems", db));
        ...
        try {
            ...
            if (stream) {
                ...
                for await (const evt of stream) {
                    buildState(products, allTags, evt, setProducts, setAllTags, setOrderItems, setErc20Addr, setPublishedTagId, setFinalizedOrders, setPubKeys, setStoreData, walletAddress);
                }
            }
        }
        catch (err) {
    ...
    const verify = async (_orderItems, _pubKeys) => {
    ...
    const addProduct = async (product, selectedTagIds) => {
        try {
            // @ts-expect-error FIXME
            const path = await relayClient.uploadBlob(product.blob);
            const metadata = {
                // ????
            ...
            const iid = bytesToHex(await relayClient.createItem({ price: product.price, metadata }));
            product.id = iid;
    ...
    const updateProduct = async (itemId, fields, updatedProduct, selectedTagIds) => {
        try {
            if (fields.price) {
            ...
            if (fields.metadata) {
                const hasEmbeddedImage = updatedProduct.metadata.image.includes("data:image");
                const path = hasEmbeddedImage
                    ? // @ts-expect-error FIXME
                        await relayClient.uploadBlob(updatedProduct.blob)
    ...
    const createTag = async (name) => {
        try {
            ...
            const id = bytesToHex(await relayClient.createTag(_name));
    ...
    const addProductToTag = async (tagId, itemId) => {
        try {
    ...
    const removeProductFromTag = async (tagId, itemId) => {
        try {
    ...
    const updateOrder = async (itemId, saleQty = 0) => {
    ...
    const invalidateOrder = async (msg = null) => {
        try {
            console.log(`Invalidating order: ${msg}`);
            if (!orderId)
                throw Error(`No ${orderId} found`);
            if (!relayClient)
                throw Error(`Disconnected from relayClient`);
            await relayClient.updateOrder({ orderId, orderCancelled: {} });
            setOrderItems({
                type: UPDATE_ORDER_STATUS,
                payload: { orderId: orderId, status: IStatus.Failed },
            });
            await createOrder();
        }
        catch (error) {
    ...
    const changeStock = async (itemIds, diffs) => {
        await relayClient.changeStock({ itemIds, diffs });
    };
    const commitOrder = async (isERC20Checkout) => {
        try {
