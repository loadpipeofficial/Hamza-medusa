...
import { ADD_PRODUCT, UPDATE_METADATA, UPDATE_PRICE, ADD_PRODUCT_TAGS, REMOVE_PRODUCT_TAG, UPDATE_STOCKQTY, } from "@/reducers/productReducers";
import { ADD_TAG } from "@/reducers/tagReducers";
import { bytesToHex } from "viem";
import { parseMetadata } from "@/app/utils";
import { IStatus } from "@/types";
import { UPDATE_ORDER_ITEM, REMOVE_ORDER_ITEM, UPDATE_ORDER_STATUS, UPDATE_ORDER_HASH, SET_ORDER_SIG, } from "@/reducers/orderReducers";
import { SET_ORDER, } from "@/reducers/finalizedOrderReducers";
import { SET_STORE_DATA } from "@/reducers/storeReducer";
import { ADD_KC_PUBKEY } from "@/reducers/KCPubKeysReducers";
export const buildState = (products, allTags, event, productsDispatch, tagsDisaptch, setOrderItems, setErc20Addr, setPublishedTagId, setFinalizedOrders, setPubKeys, setStoreData, walletAddress) => {
    if (event.shopManifest) {
        ...
        if (um.addErc20Addr) {
            console.log(`Adding erc20 ${bytesToHex(um.addErc20Addr)} to payment options`);
            setErc20Addr(bytesToHex(um.addErc20Addr));
        }
        else if (um.removeErc20Addr) {
            console.log(`Removing erc20 ${bytesToHex(um.removeErc20Addr)} from payment options`);
            setErc20Addr(null);
        }
        ...
        if (uo.itemsFinalized) {
            ...
            const { currencyAddr, orderHash, payeeAddr, salesTax, total, totalInCrypto, subTotal, } = uo.itemsFinalized;
            const orderObj = {
