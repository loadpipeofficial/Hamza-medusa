...
import { allTagsReducer } from "../src/reducers/tagReducers";
import { orderReducer } from "../src/reducers/orderReducers";
import { finalizedOrderReducer, } from "../src/reducers/finalizedOrderReducers";
import { pubKeyReducer } from "../src/reducers/KCPubKeysReducers";
...
describe("mockclient", async () => {
    ...
    let etErc20Addr = null;
    let publishedTagId = null;
    const events = [];
    let finalizedOrders = new Map();
    let pubKeys = [];
    function productsDispatch(action) {
        products = productReducer(products, action);
    }
    function tagsDisaptch(action) {
        allTags = allTagsReducer(allTags, action);
    }
    function setOrderItems(action) {
        orderItems = orderReducer(orderItems, action);
    }
    function setErc20Addr(add) {
        etErc20Addr = add;
        console.log(`setErc20Addr set to: ${etErc20Addr}`);
    }
    function setPublishedTagId(id) {
        publishedTagId = id;
        console.log(`publishedTagId set to: ${publishedTagId}`);
    }
    function setFinalizedOrders(action) {
        finalizedOrders = finalizedOrderReducer(finalizedOrders, action);
    }
    function setPubKeys(action) {
        pubKeys = pubKeyReducer(pubKeys, action);
    }
    function setStoreData(action) {
        storeData = storeReducer(storeData, action);
    }
    client.on("event", (e) => {
        events.push(e.request);
        buildState(products, allTags, e.events, productsDispatch, tagsDisaptch, setOrderItems, setErc20Addr, setPublishedTagId, setFinalizedOrders, setPubKeys, setStoreData);
    });
    await client.connect();
    const vectorItems = client.vectors.reduced.items;
    ...
    const testTagsMap = new Map(Object.entries(client.vectors.reduced.manifest.published_tag));
    ...
    test("Correctly builds a map of all products", () => {
        ...
        expect(products.get(productsMapKeysArr[0]).price).toEqual(testItemsMap.get(testDataKeysArr[0])?.price);
        const testData = testItemsMap.get(testDataKeysArr[0])?.metadata;
        const productsData = products.get(productsMapKeysArr[0]).metadata;
        expect(productsData.title).toEqual(JSON.parse(testData).title);
        //second item
        const testData2 = testItemsMap.get(testDataKeysArr[1])?.metadata;
        const productsData2 = products.get(productsMapKeysArr[1]).metadata;
        expect(products.get(productsMapKeysArr[1]).price).toEqual(testItemsMap.get(testDataKeysArr[1])?.price);
    ...
    test("Correctly builds a map of all tags", () => {
        ...
        expect(testTagsMap.get(idToHex)?.text).toEqual(allTags.get(firstTagId).text);
    });
    test("Add a product to tag", () => {
        ...
        expect(firstItemTagIds[0]).toContain(testItemsMap.get(testDataKeysArr[0])?.tag_id[0]);
    ...
    test("Remove product from tag", () => {
        expect(products.get(productsMapKeysArr[1]).tagIds).toEqual(testItemsMap.get(testDataKeysArr[1])?.tag_id);
    });
    test("Create order", () => {
        const testDataOrderObj = client.vectors.reduced.open_orders;
        const testDataFirstOrderId = Object.keys(testDataOrderObj)[0];
        const testDataFirstOrderItemId = Object.keys(testDataOrderObj[testDataFirstOrderId])[0];
        const orderExists = orderItems.get(`0x${testDataFirstOrderId}`);
        expect(orderExists.items).toBeTruthy();
        expect(orderExists.items[`0x${testDataFirstOrderItemId}`]).toEqual(testDataOrderObj[testDataFirstOrderId][testDataFirstOrderItemId]);
    });
    test("Change stock of item", () => {
        expect(products.get(productsMapKeysArr[0]).stockQty).toEqual(testItemsMap.get(testDataKeysArr[0])?.stock_qty);
        expect(products.get(productsMapKeysArr[1]).stockQty).toEqual(testItemsMap.get(testDataKeysArr[1])?.stock_qty);
    });
});
