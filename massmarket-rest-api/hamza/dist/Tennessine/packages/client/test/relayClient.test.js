...
import { bytesToHex, hexToBytes, createWalletClient, createPublicClient, http, } from "viem";
...
const relayEndpoint = (process && process.env["RELAY_ENDPOINT"]) || "ws://localhost:4444/v2";
let relayClient;
...
describe("RelayClient", async () => {
    ...
    test("regstrationTokenRedeem", async () => {
        ...
        const hash = await relayClient.blockchain.createInviteSecret(wallet, token.address);
        // wait for the transaction to be included in the blockchain
        const receipt = await publicClient.waitForTransactionReceipt({
            hash,
        });
        ...
        const hash2 = await relayClient.blockchain.redeemInviteSecret(sk, client2Wallet);
        // wait for the transaction to be included in the blockchain
...
describe("user behaviour", () => {
    ...
    test("update shop manifest", async () => {
        ...
        await relayClient.updateShopManifest({
            addAcceptedCurrency: {
                tokenAddr: hexToBytes(abi.addresses.Eddies),
                chainId: 44,
            },
        });
        await relayClient.updateShopManifest({
    ...
    test("blob upload", async () => {
        ...
        expect(result.ipfs_path).toBe("/ipfs/QmcJw6x4bQr7oFnVnF6i8SLcJvhXjaxWvj54FYXmZ4Ct6p");
    });
        ...
        describe("checkout process", () => {
            ...
            test("single item checkout", { timeout: 10000 }, async () => {
                await relayClient.updateShopManifest({
                    ...
                    addPayee: {
                        addr: hexToBytes(abi.addresses.Eddies),
    ...
    describe("invite another user", { retry: 3 }, async () => {
        ...
        beforeEach(async () => {
            ...
            const redeemHash = await relayClient.blockchain.redeemInviteSecret(sk, client2Wallet);
            // wait for the transaction to be included in the blockchain
