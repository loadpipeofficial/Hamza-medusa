...
import schema, { MESSAGE_TYPES, MESSAGE_PREFIXES, } from "@massmarket/schema";
...
export class RelayClient extends EventEmitter {
    ...
    keyCardEnrolled;
    eventStream;
    constructor({ relayEndpoint, keyCardWallet, keyCardEnrolled, shopId, }) {
        super();
    ...
    encodeAndSend(encoder, object = {}) {
        ...
        typed[0] = MESSAGE_TYPES.get(encoder);
        typed.set(payload, 1);
    ...
    async #decodeMessage(me) {
        ...
        console.log(`[recv] reqId=${bytesToHex(message.requestId)} typeCode=${prefix}`);
        switch (pbMessage) {
    ...
    async connect() {
        if (!this.connection ||
            ...
            this.connection.addEventListener("message", this.#decodeMessage.bind(this));
        }
        return new Promise((resolve, reject) => {
    ...
    async uploadBlob(blob) {
        await this.connect();
        const uploadURLResp = (await this.encodeAndSend(schema.GetBlobUploadURLRequest));
