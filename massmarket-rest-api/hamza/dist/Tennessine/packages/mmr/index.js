...
export default class MMR {
    ...
    get(index) {
        if (index > this._size)
            throw new Error("the given index is larger the number of items in the tree");
        ...
        for (let index of indexes) {
            ...
            while (path.length) {
                ...
                if (preNode - 1 === curNode) {
                    ...
                    if (!internalPos.has(leftSibling))
                        proofPos.add(leftSibling);
                }
                else {
    ...
    static _pathToLeaf(leafIndex, size) {
        let path = [];
