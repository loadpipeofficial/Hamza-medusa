...
export class RelayClient extends EventEmitter {
    ...
    async enrollKeycard(wallet: any) {
    ...
    async createInviteSecret(wallet: any) {
        if (!this.storeId)
    ...
    async redeemInviteSecret(secret: `0x${string}`, wallet: any) {
        if (!this.storeId)
